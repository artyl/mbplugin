...
## mbplugin v1.00.03 (??.10.21) Bugfix
...
ADD: Номер у которого никогда не менялся баланс теперь показывается как будно не именялся 999 дней (спасибо d1mas)  
FIX: При работе в unix-like системах, если путь с symlink возникали проблемы.  
...
CHANGE: Docker отрабарыевает под uid текущего пользователя  
ADD: Добавлены стоп слова по которым ищем нежелательные услуги (subscribtion_keyword)  
