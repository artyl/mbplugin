...
## mbplugin v1.00.85 (07.11.25) fix tele2
FIX: TELE2 востановлен запрос баланса  

## mbplugin v1.00.86 (13.12.25) workaround mts qrator captcha
...
## mbplugin v1.00.87 (19.01.26) mts link hack and last browser
CHANGE: Основной страницей захода в ЛК сделал страницу с тарифом, на удивление оказалось действенно, но подозреваю что этой лазейки надолго не хватит  
CHANGE: Браузер со страницы View profile пытаемся открыть браузером, которым последний раз делали опрос  

## mbplugin v1.00.88 (15.02.26) mts, tele2, shell from page
FIX: tele2 поменял url страницы с балансом  
FIX: mts исправлено получение баланса, если есть кэшбэк (code ММА, спасибо коллеге feoser за изыскания)  
ADD: Добавлен параметр `tg_enable_notification`, по умолчанию сообщения отправляются в беззвучном режиме даже если в канале включен звук, нет необходимости мьютить канал, но если канал не замьючен, то появляются и уведомления, так что смысл мьютить остается  
CHANGE: mbp browser и http profile открывают профиль тем браузером которым он открывался последний раз (из файла Last Browser)  
ADD: Запуск консольного shell и python из браузера (только локально) со страницы main  

## mbplugin v1.00.89 (01.03.26) mts balance2 and remains
ADD: mts в balance 2 записывается ММА - PA-OF  
FIX: восстановил получение остатков в МТС  
FIX: zadarma/novofon поменял название поля логона, поправил. (вдобавок при входе хочет капчу, так что логинится все равно надо или с включенным хромом или вручную)  

## mbplugin v1.00.90 (30.03.26) New logo
ADD: Новая иконка программы  
CHANGE: Изменен расчет абон платы на beeline и tele2  
CHANGE: Из опроса билайн убрал wait_until=commit, т.к. на win10+ это перестало влиять на скорость работы, а на win7 ломало опрос  
FIX: tele2 убрал роуминговые минуты из расчета минут  
CHANGE: Изменения в логировании для уменьшения лога в случае одинаковых повторяющихся ошибках  

## mbplugin v1.00.91 (06.07.26) Small bugfix
ADD: Параметр tg_api_urls для перенаправления бота  
FIX: Фильтр в механизме логирования добавленный в версии .90 в ряде случаев приводил к тому что бот переставал отвечать  
FIX: Вызов page_screenshot мог вызывать падение по таймауту, что приводило к ошибке опроса в плагине билайна  
FIX: Исправлен расчет даты окончания тарифа в МТС  
FIX: Исправлена ошибка получения данных из ЛК МТС, в случае tariffPricesInfo=None (добровольная блокировка или на каких-то новых тарифах) запрос аварийно завершался  