...
BREAKING_CHANGE: из requirements*.txt удалил неявные зависимости, по факту получается что от их присутствия больше вреда чем пользы и обновил до самой свежей версии, понимаю что это  сломает Dockerfile_arm (как смог поправил), но проверить не могу, так что если неправильные версии/ссылки то жду pull request  
...
BUG: 10.04.2023 МТС улучшил механизмы антифрода, в результате чего перестал разрешать запросы от mbplugin.  
FIX: Механизм работы полностью переделан, и хотя работает, но пока только в режиме обычного браузеза без headless режима и крайне нестабильно  
