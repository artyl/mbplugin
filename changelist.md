...
## mbplugin v1.00.03 (23.10.21) Docker, subscribtion alert, MTS API mode, beeline web mode, new plugin
...
ADD: Номер у которого никогда не менялся баланс теперь показывается как будто не именялся с даты первого опроса (спасибо d1mas)  
...
ADD: tele2 mbplugin и классический плагин в рассчет по услугам добавляются подписки  
...
ADD: Добавлены стоп слова по которым ищем нежелательные услуги (subscribtion_keyword) и выдаем предупреждение  
...
## mbplugin v1.00.05 (??.11.21) Fix beeline, fix ini editor
FIX: Оказалось не во всегда по билайну есть отатки по интернету в ответе, плагин билайна на таких случаях падал и не доходил до конца - исправлено  
FIX: Несколько исправлений по работе реджактора ini. Все еще очень сырой, так что если будете пробовать - использовать с осторожностью  
