...
## mbplugin 0.99.32 (15.06.21) Актуализация
...
## mbplugin 0.99.33 (??.??.21) Playwright + linux
FIX: Исправлена работа плагина megafonb2b.py  
Standalone. Шедулер для автоматического получения балансов (параметр schedule в ini, примеры см в settings.py)  
В этом релизе много внутренних изменений, которые дают кроссплатформенность и стабильность. Для совместимости с linux и mac было выпелено много кода завязанного на windows.  
Движок работы через браузер изменен с [pyppeteer](https://github.com/pyppeteer/pyppeteer) на [playwright](https://github.com/microsoft/playwright-python)  (причины перехода, низкая активность автора pyppeteer, более стабильная работа playwright, активность команды, наличие отладчика, асинхронный режим работы и много чего еще хорошего)  
Теперь __для работы не требуется установленный хром__, по умолчанию используется движок chromium загружаемый при установке. Внимание при первом запуске setup_and_check скачивается chromium - это примерно 100 МБ трафика, при желании можно прописать использование установленного в систему хрома.  
Режим headless включен по умолчанию - браузер запускается в скрытом режиме и не происходит ни переключений фокуса окон, ни кратковременного появления окна браузера на экране, но данный режим имеет особенности - запущенный таким образом браузер уже не может быть отображен, так что если ожидается появление капчи, то придется запускать в старом режиме без headless, еще есть проблемы с некоторыми сайтами в режиме headless (на настоящий момент пока такой только ростелеком) они категоричести не хотят работать в таком режиме.  
...
В этом релизе я принял решение, что для того чтобы не объяснять как включить логирование каждый раз в случае возникновения проблем, а в headless режиме не видно что происходит в браузере запись запросов по умолчанию включена (log_response=1). Это не должно создавать проблем т.к. эти данные при каждом запросе перезатираются и не растут. Если это напрягает можно выключить  
...
Также в версии для linux и Mac мне пока не до конца понятен правильный путь установки версии для linux
Для упрощения структуры кода (я уже и сам начал путаться где что) путь к mbplugin (path в разделе MobileBalance) больше не используется, считаем что папка mbplugin находится в папке где находится phones.ini  
