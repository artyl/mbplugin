...
cd "%~dp0"
echo Проверяем что все работает JSMB LH PLUGIN простой плагин
...
cd "%~dp0"
echo Проверяем что все работает JSMB LH PLUGIN через Chrome
python\python -c "import re,requests;url=re.findall(r'(?usi)(http://127.0.0.1:.*?/)',open('jsmblhplugin\\p_test3_localweb.jsmb').read())[0];print(requests.session().get(url+'getbalance/p_test3/demo@saures.ru/demo/789').content.decode('cp1251'))"
