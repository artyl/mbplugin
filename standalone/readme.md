...
Если Вы хотите иметь доступ ко всем возможностям (Telegram bot, просмотр логов, расписание проверок и т.п.), то необходимо запускать web сервер, он легковесный, и ресурсов в состоянии покоя много потреблять не должен. 
...
3.  * Распаковываем в эту папку содержимое архива (папка mbplugin). Для windows удобнее всего скачать mbplugin.n.nn.nn.zip готовую сборку из релизов на github [github](https://github.com/artyl/mbplugin/releases) или из форума [4pda](https://4pda.to/forum/index.php?showtopic=985296&st=140#entry97043755), для linux и mac достаточно исходников, так что можно воспользоваться mbplugin_bare.n.nn.nn.zip (там же в Latest release Assets)
    * устанавливаем через git (git clone https://github.com/artyl/mbplugin)
4. Подготавливаем окружение (см ниже соответствующий раздел по своей системе)
5. Для установки:
    * Windows запускаем `mbplugin\standalone\mbp.bat`
    * Linux и Mac через docker - запускаем `bash mbplugin/standalone/mbp docker`
    * Linux и Mac без docker - запускаем `bash mbplugin/standalone/mbp python`
...
6. Настраиваем телефоны в `phones.ini`, при установке будет скопирован файл примера из mbplugin\standalone с двумя примерами телефонов. Если у вас уже есть `phones.ini` положите его в папку вместо `phones.ini` с примером и настройте пароли в поле `password2`  
...
В случае с Windows в архиве дистрибутива уже содержится все необходимое.    
...
sudo apt-get install python3.8 python3-venv npm
