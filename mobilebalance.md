...
для интеграции с базой MDB необходимо установить 32-битный ODBC драйвер для MDB AccessDatabaseEngine.exe [Скачать можно отсюда](https://www.microsoft.com/en-us/download/details.aspx?id=54920). Для первоначального полного импорта используйте plugin\copy_alldata_from_mdb.bat Замечание для пользователей автономной версии, импортировать данные можно и в ней (положив mdb файл в ту же папку, где лежит sqlite файл), но для этого нужно на время импорта добавить параметр updatefrommdb = 1, а после импорта или выключить или убрать.
