...
def get_balance(login, password, storename=None):
    ...
    def check_or_get_bearer(session):
        ...
        if response2.status_code == 200:
            ...
            return session
        logging.error(f'Bearer get error {response2.status_code} for login {login}')
    ...
    response = session.get(f'https://api.tele2.ru/api/subscribers/7{login}/balance')
    result['Balance'] = get_from_js(response , 'value')  # баланс
    response = session.get(f'https://api.tele2.ru/api/subscribers/7{login}/tariff')
    result['TarifPlan'] = get_from_js(response, 'frontName') # тариф
    response = session.get(f'https://api.tele2.ru/api/subscribers/7{login}/profile')
    result['UserName'] = get_from_js(response, 'fullName') # ФИО владельца
    response = session.get(f'https://api.tele2.ru/api/subscribers/7{login}/rests')
