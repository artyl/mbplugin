...
login_url = 'https://my.novofon.com/auth/'
# Личный кабинет переехал с https://my.zadarma.com/ сайт работает на старых адресах, но авторизация по новым ссылкам
...
def get_balance(login, password, storename=None, **kwargs):
    ...
    response3 = session.get('https://my.novofon.com/')
    if re.search(re_balance, response3.text):
        ...
        response1 = session.get('https://my.novofon.com/auth/')
        _ = response1
        # Вроде как передает в POST но работает и без этого, выкидывать не буду, но и использовать пока тоже
        # csrf_k = re.findall(r"check_user_param = '(\S+)'", response1.text)[0]
        # csrf_v = re.findall(r"check_user_val = '(\S+)'", response1.text)[0]
        # data[csrf_k] = csrf_v
        # response2 = session.post('https://my.zadarma.com/auth/login/', data=data)
        response2 = session.post('https://my.novofon.com/auth/login/', data=data)
        _ = response2
        response3 = session.get('https://my.novofon.com/')
