...
class UpdaterFromInternet():
    'Обновление через интернет с проверкой подписи файлов'
    ...
    def version_check_zip(self, zipname, ignore_crlf=True, ignore_missing=True):
    ...
    def version_update_zip(self, new_zipname):
        'Обновляет файлы на диске из архива'
        z_new = self.read_zip(store.abspath_join(new_zipname))
    ...
    def rename_new_to_current(self, new_zipname, current_zipname, current_bak_zipname, undo=False):
    ...
    def read_zip(self, zipname) -> typing.Dict[str, ZipRecord]:
        ...
        with zipfile.ZipFile(store.abspath_join(zipname), 'r') as zf1:
            for zi in zf1.infolist():  # Во временную переменную прочитали
                ...
                fn = store.abspath_join(
                    'mbplugin', *(store.path_split_all(zi.filename)[1:]))
                if store.abspath_join(zi.filename)[1] != 'plugin' or '.ico' in zi.filename:  # TODO !!! for debug
                    if not zi.is_dir():
                        res[fn] = ZipRecord(
                            zf1.read(zi),
    ...
    def install_update(self, version, force=False, undo_update=False, by_current=False):
        ...
        new_zipname = store.abspath_join('mbplugin', 'pack', f'mbplugin_bare.{version}.zip')
        ...
        if not os.path.exists(current_zipname) and not force:
            ...
            return False, f'Not exists {current_zipname} (use -f)'
        if os.path.exists(current_zipname):
            # Проверяем что нет файлов которые отличаются от релизных чтобы не перезатереть чужие изменения
            diff_current1 = self.version_check_zip(current_zipname, ignore_missing=True)
            ...
            diff_current2 = self.version_check_zip(current_zipname, ignore_missing=False)
        ...
        if os.path.exists(new_zipname) and os.path.exists(current_zipname) and not force:
            if self.read_zip(new_zipname) == self.read_zip(current_zipname):
        ...
        if os.path.exists(new_zipname) and not by_current and not undo_update:
            diff_new = self.version_check_zip(new_zipname, ignore_missing=False)
            if len(diff_new) > 0:
                # Установка new.zip
                if force or len(diff_current1) == 0:
                    print('Update:\n'+'\n'.join(diff_current2))
                    self.version_update_zip(new_zipname)
                    self.rename_new_to_current(new_zipname, current_zipname, current_bak_zipname)
                    return True, f'Update to version {store.version()} complete'
            else:
                self.rename_new_to_current(new_zipname, current_zipname, current_bak_zipname)
                return True, f'Your version is up to date with {new_zipname}'
        ...
        if undo_update and os.path.exists(current_bak_zipname):
            self.version_update_zip(current_bak_zipname)
            self.rename_new_to_current(new_zipname, current_zipname, current_bak_zipname, undo=True)
