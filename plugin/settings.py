...
ini = {
    'Options': {  # Раздел mbplugin.ini [Options]
        ...
        'requests_proxy': '',
        # коэфициент замедления delay timeout работы браузера, если 1 - не замедляем, если 2 - замедляем в 2 раза, если 0.5 - ускоряем 2 раза
        'slowdown_': {'descr': 'Коэфициент замедления', 'type': 'float', 'validate': lambda i: re.match(r'^\d+(\.\d+)?$', str(i))},
        'slowdown': '1',
        ...
        # токен для анализа капчи через сервис распознавания капчи
        'captcha_api_token_': {'descr': 'Токен для анализа капчи через сервис распознавания капчи https://rucaptcha.com, если пусто - не использовать', 'type': 'text'},
        'captcha_api_token': '',
        # Максимальное количество капч в сутки для распознавания через сервис, 0 - неограничено
        'captcha_token_limit_' : {'descr': 'Максимальное количество капч в сутки для распознавания через сервис, 0 - неограничено', 'type': 'text', 'validate': lambda i: i.isdigit()},
        'captcha_token_limit': '100',
        # логировать капчу
        'captcha_logging_': {'descr': 'Логировать капчу', 'type': 'checkbox'},
        'captcha_logging': '0',
