...
login_url = 'https://lk.mts.ru/tarif'
...
def get_balance(login, password, storename=None, wait=True, **kwargs):
    ...
    pd.send('Page.navigate', {'url': 'https://id.mts.ru'})
    time.sleep(4 * pd.slowdown)
    ...
    if state == 'lk':
        ...
        services_2 = [(s, float(str(c).replace(',', '.')) * (30 if '/день' in p else 1)*(0 if any([cs for cs in no_count_services if s.startswith(cs)]) else 1)) for s, d, c, p in services_]
        # result['BlockStatus'] = active.get('data', {}).get('accountBlockStatus', '').replace('Unblocked', '')
        try:
