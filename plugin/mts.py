...
USEDBYME = False # true - показывать использованный трафик (usedByMe) или false - показывать оставшийся трафик (NonUsed)
...
def get_balance(login, password, storename=None):
    ...
    if 'userProfile' not in response_json:
        ...
        if 'userProfile' not in response_json:
            ...
            store.drop_and_create_session(storename)
    ...
    if 'counters' in data:
        ...
        if internet != []:
            ...
            if not USEDBYME and nonused != []:
                result['Internet'] = round(nonused[0]*unitMult/unitDiv, 2)
            if USEDBYME and usedbyme !=[]:
