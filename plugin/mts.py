...
def get_api_json(session, token, longtask=False):
...
def get_balance(login, password, storename=None):
    ...
    if 'json' in response3.headers.get('content-type'):
        ...
        do_login(session, login, password)
    
    response_json = get_api_json(session, 'login/userInfo', longtask=False)
    if 'userProfile' not in response_json:
        ...
        response_json = get_api_json(session, 'userInfo', longtask=False)
        if 'userProfile' not in response_json:
    ...
    response_json = get_api_json(session, 'accountInfo/balance', longtask=True)
    ...
    response_json = get_api_json(session, 'services/list/active', longtask=True)
    data = response_json.get('data', {})
    if 'services' in data:
    ...
    response_json = get_api_json(session, 'sharing/counters', longtask=True)
    data = response_json.get('data', {})
    if 'counters' in data:
