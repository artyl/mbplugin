...
def api(session, token, login, item):
    ...
    if response.status_code != 200:
        raise RuntimeError(f'api get {item} error status_code {response2.status_code}!=200')
...
def get_balance(login, password, storename=None):
    ...
    uri = 'https://my.beeline.ru/api/1.0/auth/auth?login=' + login + '&password=' + password;
    response1 = session.get(uri)
    if response1.status_code != 200:
        raise RuntimeError(f'Login error: status_code {response2.status_code}!=200')
