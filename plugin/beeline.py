...
class browserengine(browsercontroller.BrowserController):
    def data_collector(self):
        ...
        bal_tag = 'api/profile/userinfo/data/?noTimeout'
        # TODO костыль - сайт очень медленно открывается тупо долго ждем
        for _ in range(0, 180, 5):
            logging.info(f'Wait page {bal_tag} {len(self.responses)}')
            if any([u for u in self.responses if bal_tag in u]):
                break
            self.sleep(5)
        else:
            raise RuntimeError(f'Так и не получили страницу с балансом {bal_tag}')
        self.wait_params(params=[
            {'name': 'Balance', 'url_tag': [bal_tag], 'jsformula': "parseFloat(data.balance.data.balance).toFixed(2)"},
            {'name': 'TarifPlan', 'url_tag': [bal_tag], 'jsformula': "data.profileSummary.data.tariffName"},
            {'name': 'Internet', 'url_tag': [bal_tag], 'jsformula': "Math.max.apply(null,data.accumulators.data.list.concat(data.accumulators.data.listForYoung).map(el => (el!=undefined&&el.unit=='KBYTE'?el.rest:0)))"},
            {'name': 'Min', 'url_tag': [bal_tag], 'jsformula': "Math.max.apply(null,data.accumulators.data.list.concat(data.accumulators.data.listForYoung).map(el => (el!=undefined&&el.unit=='SECONDS'?el.rest/60:0))).toFixed(0)"},
            {'name': 'SMS', 'url_tag': [bal_tag], 'jsformula': "Math.max.apply(null,data.accumulators.data.list.concat(data.accumulators.data.listForYoung).map(el => (el!=undefined&&el.unit=='SMS'?el.rest:0)))"},
            {'name': 'BlockStatus', 'url_tag': [bal_tag], 'jsformula': "data.status.data.status"},
            {'name': 'LicSchet', 'url_tag': [bal_tag], 'jsformula': "data.profileSummary.data.ctn"},
        ])
