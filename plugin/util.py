...
def check_import(ctx):
    ...
    try:
        ...
        click.echo(f'Import fail: {"".join(traceback.format_exception(*sys.exc_info()))}')
        return
    click.echo('Import OK')

@cli.command()
@click.argument('turn', type=click.Choice(['on', 'off'], case_sensitive=False), default='on')
@click.pass_context
def autostart_http_server(ctx, turn):
    'Автозапуск http сервера (только windows)'
...
def check_dll(ctx):
    ...
    if sys.platform == 'win32':
        try:
            ...
            click.echo(f'Check DLL plugin OK')
        except Exception:
            click.echo(f'Fail:\n{"".join(traceback.format_exception(*sys.exc_info()))}')
    else:
        click.echo('On windows platform only')    
