...
def test_stock():
    ...
    assert currency.get_balance('MOEX EUR/RUB','123')['Balance'] > 0
    assert currency.get_balance('MOEX_currency_selt_EUR_RUB__TOM','123')['Balance'] > 0
    assert currency.get_balance('MOEX_stock_shares_AFLT','123')['Balance'] > 0
    assert currency.get_balance('MOEX_stock_shares_TATNP','123')['Balance'] > 0
